...
list <- scan_files(dir.proj = dir.proj, scan.for = c("ebird_spatial", "bbs_spatial"))
...
j.ind <- ifelse(!exists("jdat") | (exists("jdat") & overwrite.jdat), TRUE, FALSE)
...
# Step 3: Munge Data for Use in JAGS
We need to munge the data just a little bit further for use in JAGS together. You may need to create/munge data furhter depending on model(s). 

```{r jdat-ebird, eval=j.ind}
