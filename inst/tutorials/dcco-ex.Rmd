...
args.save <- junk_it(args.save, paste0(c(j.ind, e.ind, b.ind, g.ind), ".ind"))
```
...
if(!g.ind){grid <- readRDS(paste0(dir.spatial.out, "grid.rds"))}else{
...
## If b.ind is FALSE then read in bbs_spatial, otherwise, create or import the bbs observations and create bbs_spatial
if(!b.ind){ bbs_spatial <- readRDS(paste0(dir.spatial.out, "bbs_spatial.rds"))}else{
...
rm(bbs_obs, bbs_orig)    # shouldn't need these anymore so just clear up a little mem
}
...
if(!e.ind){ebird_spatial <- readRDS(paste0(dir.spatial, "ebird_spatial.rds"))}else{
  ...
  ebird_filtered <- filter_ebird_data(
      fns.ebird = id_ebird_files(dir.ebird.in = dir.ebird.in), # finds only relevant ebird obs and smapling events data in dir.ebird.in
      overwrite = FALSE,
