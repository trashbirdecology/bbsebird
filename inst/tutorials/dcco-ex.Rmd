...
args.save <- junk_it(args.save, "gamdat")
```
...
jagam.data <- bbs_spatial %>% 
  sf::st_drop_geometry() %>%  
  group_by(gridcellid, cell.lat.centroid, cell.lon.centroid) %>% summarise(maxc=max(c, na.rm=TRUE)) %>% ungroup()
jagam.data[jagam.data=="-Inf"] <- 0
...
## Save model output
saveRDS(jagam.mod, file=paste0(dir.models, mod.name, ".rds"))
