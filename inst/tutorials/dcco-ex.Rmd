...
```{r}
gam.dat <- data.frame(Ni = Ni_ebird, )
# Generate the appropriate prior distributions for a poisson gam
# jagam.data <- data.frame(N = rep(N = Ni[,1], length(jdat$XY[,1])),
#                          x=scale(jdat$XY[,"x"]), y=scale(jdat$XY[,"y"]))
jagam.mod <- mgcv::jagam(N ~ s(x, y, # Grid cell centroid coordinates
                               #  number of basis functions (controls upper
                               # level of 'smoothness')
                               k = 20,
                               bs = 'ds',    # duchon spline
                               # duchon spline w/ 1st deriv penalty
                               # (penalizes wiggliness)
                               m=c(1, 0.5)),
                         # use log.uniform priors on the lambda penalty term
                         sp.prior = "log.uniform",
                         # Smooths re-parameterized to have i.i.d. Gaussian
                         # priors (where possible)
                         diagonalize = TRUE,
                         data = jagam.data, family = "poisson",
                         file=system.file("./jags/", "jdpois_tp.jags"))
```
