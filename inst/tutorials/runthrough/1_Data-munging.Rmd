...
```{r specs-directories, echo=TRUE}
## Create a shorthand name for the project
# proj.shorthand="fl-2008-2019"
```
...
## establish conditions for skipping data munging chunks if data already exists for this example!
 ...
 }else(eval_bbs <- eval_ebird<-eval_grid<-eval_combo<-TRUE)
```
...
args.save <- junk_it(args.save, c("bbs","grid","ebird"))
```
Although the data are ready for use and technically integrate-able, we need to munge the data just a little further.
...
Create arrays descriving the covariates influencing detection probabilities.
...
Create some indexing on the BBS data for JAGS
...
n.routesperyear <- n.routesperyear$n # tibbles are annoying in this way..
```

Create arrays for BBS count data
...
## before creating observations at the route level, ensure the empty data (i.e. RTENO is NA) are gnoe
bbs.df <- bbs.df %>% filter(!is.na(rteno))
...
```{r max-bbs-obs, eval=FALSE}
temp <- bbs %>% group_by(gridcellid) %>% summarise(max_C_bbs= max(C, na.rm=TRUE))
plot(temp["max_C_bbs"], main="Max # birds counted on a single BBS route per grid cell.")
rm(temp)
```

```{r max-ebird-obs, eval=FALSE}
# takes too long
# temp <- ebird %>% group_by(gridcellid) %>% summarise(max_C_ebird= max(C, na.rm=TRUE))
# plot(temp["max_C_ebird"], main="Max # birds counted on a single BBS route per grid cell.")
# rm(temp)
```
