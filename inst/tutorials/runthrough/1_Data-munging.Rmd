...
## Next, ensure all grid cells are represented each year even when BBS data does not exist (for JAGS)
bbs.df <- full_join(bbs.df, as.data.frame(grid) %>% dplyr::select(-geometry)) %>% 
## ensure all years are represented for new grid cells
          tidyr::complete(year, gridcellid) ## this isn't really filling in the blanks
...
Create arrays for BBS count data (dims: rteno by year by grid cell)
```{r prep-bbs-jags-count-arrays}
### C: array <rteno by year by cell id>
C.grid       <- reshape2::acast(bbs.df, rteno~year~gridcellid, value.var="C")
...
w.area <- scale(as.vector(
  reshape2::acast(
    bbs.df %>% distinct(gridcellid, cellarea),
    gridcellid ~ .,
    value.var = "cellarea"
  )
...
years <- min(bbs$year, na.rm=TRUE):max(bbs$year, na.rm=TRUE) # need to fix this!!
T     <- length(years)
...
```{r}
temp <- bbs %>% group_by(gridcellid) %>% summarise(max_C_bbs= max(C))
