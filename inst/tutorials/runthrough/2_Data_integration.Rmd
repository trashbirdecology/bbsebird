...
<!-- Create some indexing on the BBS data for potential reference in JAGS.  -->
```{r prep-bbs-jags-indexing, eval=FALSE}
## may detlee soon
...
w.prop <-
  acast(
    bbs.df.sampled %>% units::drop_units() %>% distinct(rteno, gridcellid, proprouteincell),
    rteno ~ gridcellid,
...
w.area <- scale(as.vector(
  reshape2::acast(
    bbs.df.sampled %>% distinct(gridcellid, area),
    gridcellid ~ .,
    value.var = "area"
  )
