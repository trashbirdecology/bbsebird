...
p.cars      <-  acast(bbs.df %>% distinct(rteno, year, car.z), rteno~year, value.var = "car.z", drop = TRUE)
...
R = dim(C.route)[1] # <route> number of unique BBS routes with data
...
Create a matrix comprising the grid cell identities that a rteno goes through. Since this doesn't change over time, we won't add the time dimension. 
```{r }
temp <- bbs.df %>% distinct(gridcellid, rteno) %>% 
  group_by(rteno) %>% 
  mutate(grid.order.num = 1:n_distinct(gridcellid)) %>% 
  ungroup()
grid.ids.by.year <- acast(temp, rteno~grid.order.num, value.var="gridcellid")
# dim(ids.grids.by.year) ## dims: [num routes by max-num grids a single rteno goes through]
rm(temp)
```

## For indexing on BBS data
```{r indexing-bbs}
rteno.fac     <- rownames(C.grid) %>% as.factor()
rteno.fac     <- data.frame(rteno=rteno.fac, 
                             rteno.fac=as.numeric(rteno.fac))
cellids        <- bbs.df.sampled %>% distinct(rteno, gridcellid) %>% 
  mutate(cellid.fac = as.numeric(as.factor(gridcellid))) 
cellids <- full_join(cellids, rteno.fac) %>% arrange(rteno.fac, cellid.fac)
...
names.bbs <-
  c(
    "bbs.df", ## data in full
    "R", # number of unique routes across entire dataset
    ...
    "w.area",
    "w.prop",
    "grid.ids.by.year", ## gridcellid identities that a single rteno goes through. colnames unimportant. 
    "p.obsfyrbbs",
    "p.obsfyrrteno",
    "p.cars",
    "p.noise",
...
```
Wrap  all the BBS data up into a nice little JAGS list.
```{r make-bbs-jdat}
jdat.bbs <- list()
...
args.save <- junk_it(args.save, "jdat.bbs")
```
