...
C.grid.sums <- t(colSums(aperm(C.grid, c(1,2,3)), na.rm=TRUE))## probbaly shorter way of doing this with aperm and no transpose.
dim(C.grid.sums)
...
for(r in 1:R){
  for(t in 1:T){  
    ...
    ### call vals in R for testing
    # r=40;t=3;g=13 ## this should have C=11 at this r,t,g
      y[r,t] ~ dpois(lambda.route[r,t])
      lambda.route[r,t] <-  C.grid.weighted.sum[g,t] 
          ## sum across all gs of weighted grid count
      
      log(C.grid.weighted.sum[g,t]) <- log(area[g]) + alpha[g] + beta1[g]*grid.cov1[g] 
          ##lambda[g] = expected count for a sample taken in grid cell g
