...
### whenever we get ebird data settled in we should go write the data structures to file. 
```
...
## Initial values
```{r set-inits} 
# repeat inits for number of desired chains
inits <- list(
...
params.monitor <- c("lambda.route", "nu", "alpha", "beta1", "N")
```
...
C.grid <- jdat$bbs$C.grid # raw counts sliced by grid per route year
...
names.jags <- c(
  "w.prop", 
  "y", 
  # "C.grid", # raw counts sliced by grid cells 
...
for(r in 1:R){
  for(t in 1:T){  
    for(g in 1:G){ 
      ...
      lambda[r,t] <- inprod(w.prop, nu) # w.prop dims=[r,g]; nu dims=[g]
      
      log(nu[g]) <- area[g] + alpha + beta1*grid.cov1[g] 
          # note havent taken log of area -- need to check on previous scaling
