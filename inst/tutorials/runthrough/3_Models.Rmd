...
jdat$bbs$grid.ids.by.year %>% head()
```

Get the dimensions of the BBS data objects in jdat
```{r data-structure-bbs}
# bbs.structure <- get_data_structure(listname)
# for reference
# View(bbs.structure)
...
grid.ids.bbs <- jdat$bbs$grid.ids.by.year
gmax.bbs.index <- rowSums(!is.na(grid.ids.bbs)) # another index used to avoid summing up NA values from grid.ids.bbs and gmax.bbs
gmax.bbs <- max(gmax.bbs.index, na.rm=TRUE) ## an index used in JAGS to calculate nu (lambda.grid)
```
...
for(r in 1:R){
  for(t in 1:T){  
    ...
    lambda[r] <- sum(lambda.temp)
    
    # JAGS cannot handle summing ragged vectors-> nasty workarounds.
    for(z in 1:gmax.bbs.index[r]){
         lambda.temp[z] <- w.prop[r,grid.ids.bbs[r,z]]*nu[grid.ids.bbs[r,z]]
        # cell.index[z] <- grid.ids.bbs[r,z]
    
    for(g in 1:G){ 
        log(nu[g]) <- area[g] + alpha[g] + beta1[g]*grid.cov1[g]# note log not taken
      } # end G (grid cell)
      
    } # end z (gmax.bbs.index) 
