...
yBBS           <- make_array(bbs, val="C")

### vectors comprising site, cell, and year identities
site.ids.bbs   <- dimnames(yBBS)[1] %>% unlist() %>% as.factor()#sites=rteno
year.ids.bbs   <- dimnames(yBBS)[2] %>% unlist() %>% as.integer()#years
grid.ids.bbs   <- dimnames(yBBS)[3] %>% unlist() %>% as.numeric()#gridcellid
### indexes associated with id vectors for site, cell, and year
site.ind.bbs     <- 1:dim(yBBS)[1]
year.ind.bbs     <- 1:dim(yBBS)[2]
grid.ind.bbs     <- 1:dim(yBBS)[3]
...
Create matrix containing the proportion of routes in each grid cell
```{r bbs-site-prop-in-cell}
...
p.bbs.yday    <- bbs$yday ### may want to change this to julian date
p.bbs.jday    <- bbs$julian ### may want to change this to julian date
```

Specify desired inputs into JAGS data output for BBS 
```{r names.bbs}
names.bbs <- c(
 # route-level detection covariates
 # 'p.bbs.wind', 
 # 'p.bbs.cars', 
 # 'p.bbs.noise', 
 # 'p.bbs.yday', 
