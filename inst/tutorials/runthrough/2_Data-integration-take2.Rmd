...
Create array comprising all routes identities for a grid cell
```{r}
temp <- bbs %>% distinct(gridcellid, rteno) %>% merge(bbs.index) %>% 
  distinct(site, grid, year) %>% 
  group_by(grid, year) %>% 
  mutate(site.order.num = 1:n_distinct(site)) %>% ## site.order.num counts and tags the first, second..nth route that lies in a grid each year (time matters since not all routes are obs. every year)
  ungroup()

sites.in.grid.bbs <- acast(temp, grid~site.order.num~year, value.var="site") 
rownames(sites.in.grid.bbs) <- grid.ids.bbs # rows=grid cell identity , where nrow=G.bbs; cols=first, second ...nth RTENO that is in that grid cell; slice=year

## so i think together we can use SITESIN GRIDBBS + bbs.index to say
# grab an index to determine the number of iterations over a grid's `site.order.num` for est. lambda.grid (nu)
num.sites.per.grid <- 
  rowSums(!is.na(sites.in.grid.bbs))
## need to do that for each array, though^^^
C.grid.sums <- t(colSums(aperm(jdat$bbs$C.grid, c(1,2,3)), na.rm=TRUE))## probbaly shorter way of doing this with 
dim(sites.in.grid.bbs)# dims: g,numsites,t
