...
title: "0_setup"
params:
  species:  "HOSP, House Sparrow, House-Sparrow"
...
j.ind <- ifelse((!exists("jdat") &  j.temp)| (exists("jdat") & overwrite.jdat), TRUE, FALSE)
b.ind <- ifelse(!exists("bbs_spatial") | (exists("bbs_spatial") & overwrite.bbs), TRUE, FALSE)
e.ind <- ifelse(!exists("ebird_spatial") | (exists("ebird_spatial") & overwrite.ebird), TRUE, FALSE)
...
# Step 3: Munge or Import JAGS Data
...
## Step 4a: Computational Specifications
### MCMC
```{r set-mcmc}
...
for(s in 1:S.bbs){
  lambda[s] = inprod(nu[], prop.site.in.cell.bbs[s,]) ## check inprod
} # end s (lambda route)
...
# Step 5: Run JAGS
```{r run-jags}
# browseURL(mod.fn) ### inspect .jags model
remove=c("yeBird.site", "G.ebird", "T.ebird", "S.ebird" ,"XY", "G")
jags.data <- within(jdat, rm(list=remove))
# names(jags.data)
