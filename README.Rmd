...
dirs        <-  dir_spec(dir.orig.data, dir.proj,subdir.proj) # create and/or specify 
```
...
`bundle_data` creates site-level covariates in both long (vector) and wide (matrix) form. Matrix form are housed inside Xsite matrix, whereas long-form are within bbs.df and ebird.df.
```{r bundle_data}
...
## Step 2: bugs.data
The output of `bundle_data()` will likely create more constants and data than the user requires. Extract what is necessary for your model here. You can unwrap this from a function, but the function makes it quicker to switch between the dev and full bundled datasets.
```{r make bugs.data}
...
params <- c("alpha1",   #bbs asst
            "alpha2",   #bbs fyr observer
            "yeareff",  #bbs year effects
            "obseff",   #bbs observer effects
            "beta1",    #grid hab
            "alpha11",  #ebird num in birding party
            "alpha22",  #ebird dist
            "alpha33",  #ebird mins
            "alpha44",  #ebird effarea
            "alpha55",  #ebird starttime
            "Ntot" , 
            "lambda" 
)
```
...
set.inits <- function(n = 1, dat = bugs.data) {
  ...
  for (i in 1:n) {
    inits.list <- list(
      ...
      obseff        = rep(rnorm(dat$nobsrb, 0, 0.001)),
...
myMCMC.specs <- set_mcmc_specs(dev.mode=FALSE, ni=25000)
...
results.jags <- run_in_jags(
  ...
  monitor = params, 
  mod.name = "hosp-fl-run", 
  overwrite=TRUE, 
  traceplots=TRUE)
tictoc::toc()
# browseURL(list.files(dirs$dir.results, ".pdf", full.names=TRUE)[2])
```
...
bugs.dat  <- make.bugs.data(bundle.dev)
# bugs.dat  <- make.bugs.data(bundle)
myMCMC.specs <- set_mcmc_specs(dev.mode=FALSE, ni=50000)
results.nimb <- run_in_nimble(
  myModel = mod.fn,
  myData = bugs.dat,
  myInits = set.inits(n=1, dat=bugs.dat),
  # myInits = inits,
  parallel = FALSE, # suggest FALSE when debugging/dev mode
  mcmc.specs = myMCMC.specs,  
  monitor = params, 
  mod.name = "testrun", 
  overwrite = TRUE)
library(coda)
coda.samples <- as.mcmc(results.nimb)
```
