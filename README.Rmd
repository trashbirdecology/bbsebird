...
```{r, include = FALSE}
eval.mods <- params$eval.mods
eval.ind <- ifelse(test=params$devmode, yes=TRUE ,no=FALSE) 
...
```{r results=FALSE, echo=TRUE, warning=FALSE, message=FALSE, eval=TRUE}
...
```{r, echo=TRUE, warning=FALSE, message=FALSE, result=FALSE, eval=TRUE}
# devtools::install_github("trashbirdecology/dubcorms")
library(dubcorms)
```
