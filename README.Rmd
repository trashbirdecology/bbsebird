...
The purpose of this R package (*likely to undergo a name change...*) is to:
  ...
  1. integrate the BBS and eBird observation datasets for use in hierarchical modeling efforts by assigning point-referenced data to a gridded surface. 
  1. provide modeling workflow for analysing spatio-temporal dynamics in Nimble or JAGS.   
...
Create the BBS data. This chunk relies on R package \code{bbsAssistant}. The resulting data is aligned with the spatial grid (see above).
```{r bbs-data}
...
bbs_spatial <- make_bbs_spatial(
  ...
  print.plots = FALSE, ## if extent large or ncells very high, do not plot..takes longer than it takes to create the overlay... 
