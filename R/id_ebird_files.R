...
id_ebird_files <-function(dir.ebird.in,
  ...
  rc.temp <- tolower(x=gsub(rc$iso_3166_2, pattern="-", replacement=""))
  states.ind <- gsub(x=tolower(states.ind), pattern = "-", replacement="")
  ...
  if(!is.null(species.abbr) & !get.full.data){
    if(is.null(states.ind)){region=country.spp}else{region=tolower(paste(states.ind, collapse="|"))}
      fns_obs.zip <- fns_obs[grepl(fns_obs,pattern= ".zip")]# possibles for the species
      fns_obs.zip <- fns_obs.zip[grepl(fns_obs.zip, pattern=region)]# possibles for the region
      f_obs <- NULL
  ...
  for(i in seq_along(fns_obs.zip)){
      ...
      f_txt <- f_txt[grepl(f_txt, pattern=".txt")]# yes i need to keep both greps here
...
if(get.full.data){
  ...
  f_rds <- list.files(dir.ebird.out, full.names = TRUE, pattern=".rds")


  # Step 4: Return object
  output <- c(f_samp, f_obs, f_rds)
  # first save to file
  f <- paste0(dir.ebird.out,  "ebird_filtered.rds")
  saveRDS(output,file = f)
  #return it
  cat("Filtered ebird data saved to ", f,"\n")
  return(output)
