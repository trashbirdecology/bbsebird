...
#' @param dir.plots where to save density plots of counts
...
# Munge Data --------------------------------------------------------------
    ...
    for (i in seq_along(dat)) {
      ...
      if (ind == "ebird") {
        ebird <- dat[[i]]
        if ("sf" %in% class(ebird))
          ebird <- ebird %>% sf::st_drop_geometry()
        ...
        yeBird.grid   <-
          make_array(ebird %>% filter(!is.na(year)),
                     row = "checklist_id",
                     val = "c")
        yeBird.site <-
          make_mat(
            ebird %>% distinct(checklist_id, year, c) %>% filter(!is.na(checklist_id)),
            row = "checklist_id",
            col = "year",
            val = "c"
          )
        yeBird.site <-
          yeBird.site %>% select(sort(names(yeBird.site))) # use select to ensure the colnames(years) are in order...

        ## make a temp df that does not contain NAs
        temp.ebird <- ebird %>% filter(!is.na(c))
        ...
        # create matrices containng the number of unique checklist ids with data for each year and gridyear
        nChecklistsByGridYear <- temp.ebird %>%
          group_by(gridcellid, year) %>%
          mutate(nChecklistsByGridYear = n_distinct(checklist_id)) %>%
          ungroup() %>%
          distinct(gridcellid, year, nChecklistsByGridYear) %>%
          arrange(year) %>% # to be safe for dcast
          reshape2::dcast(gridcellid~year, value.var="nChecklistsByGridYear") %>%
          tibble::column_to_rownames("gridcellid")
        nChecklistsByGridYear[is.na(nChecklistsByGridYear)] <- 0

        nChecklistsByYear <- temp.ebird %>%
          distinct(year, checklist_id) %>%
          group_by(year) %>%
          mutate(nChecklistsByYear = n_distinct(checklist_id)) %>%
          ungroup() %>%
          distinct(year, nChecklistsByYear) %>%
          arrange(year)
        nChecklistsByYear[is.na(nChecklistsByYear)] <- 0
      ...
      if (ind == "bbs") {
        ...
        yBBS.site   <-
          make_mat(
            bbs %>% distinct(rteno, year, c) %>% filter(!is.na(rteno)),
            row = "rteno",
            col = "year",
            val = "c"
          )
        yBBS.site   <-
          yBBS.site %>% select(sort(names(yBBS.site))) # use select to ensure the colnames(years) are in order...
        ...
        grid.bbs <-
          bbs %>% distinct(gridcellid, rteno, .keep_all = TRUE)

        ## area of each grid cell
        if (scale.vars){area.bbs <- scale(grid.bbs$area)}else{
          area.bbs <- grid.bbs$area
        }
        ...
        prop.site.in.cell.bbs[is.na(prop.site.in.cell.bbs)] <-
        ...
        S.bbs <- length(unique(temp.bbs$rteno))
        T.bbs <- length(unique(temp.bbs$year))
        G.bbs <- length(unique(temp.bbs$gridcellid))

        # create matrices containng the number of unique checklist ids with data for each year and gridyear
        nRoutesByGridYear <- temp.bbs %>%
          group_by(gridcellid, year) %>%
          mutate(nRoutesByGridYear = n_distinct(rteno)) %>%
          ungroup() %>%
          distinct(gridcellid, year, nRoutesByGridYear) %>%
          arrange(year) %>% # to be safe for dcast
          reshape2::dcast(gridcellid~year, value.var="nRoutesByGridYear") %>%
          tibble::column_to_rownames("gridcellid")
        nRoutesByGridYear[is.na(nRoutesByGridYear)] <- 0
        nRoutesByYear <- temp.bbs %>%
          distinct(year, rteno) %>%
          group_by(year) %>%
          mutate(nRoutesByYear = n_distinct(rteno)) %>%
          ungroup() %>%
          distinct(year, nRoutesByYear) %>%
          arrange(year)
        nRoutesByYear[is.na(nRoutesByYear)] <- 0

        ### IDENTIFY DESIRED BBS OBJS AS CHARACTER STRING
        objs.bbs <-
          c(
            "prop.site.in.cell.bbs",
            ## proportion of a route in a grid cell
            "yBBS.site",
            ## observed counts for bbs
            "yBBS.grid",
            ## counts at route within grid (array)
            # "S.bbs", "T.bbs", "G.bbs", ## loop length indexes,
            "p.fyrroute",  ## detection covariates
            "p.wind",
            "p.car",
            "p.noise",
            "p.fyrbbs",
...
# Make List of New Data Output Objects ------------------------------------
...
# Create plots ------------------------------------------------------------
to.plot=names(output)[names(output) %in% c("yeBird.site", "yBBS.site")]
p<-list()
  for(i in seq_along(to.plot)){
  plot.dat <- output[[to.plot[i]]] %>%
        as.data.frame()
  plot.dat$site = rownames(plot.dat)
  plot.dat <-  plot.dat %>%
      tidyr::pivot_longer(cols=-site, values_to = "C",names_to="year") %>%
      filter(!is.na(C))
p[[i]] <- ggplot(plot.dat)+
    geom_violin(aes(x=year, y=log(C)))+
    ggtitle(paste(to.plot[i]))+
    theme_classic()
}
if(!is.null(dir.plots)){
  fn.p=paste0(dir.plots, "/observed_count_density", ".pdf")
      pdf(fn.p, width=10, height=7)
      p
      dev.off()
      browseURL(fn.p)
}else{p}
