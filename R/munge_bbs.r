...
#' if ("citation" %in% names(list)){list$citation <- NULL}
#' rd
#'
#' # TAXONOMIC SUBSETTING ----------------------------------------------------
#' ## grab relevant taxonomic names and codes for subsetting BBS by species
#' sl <- list$species_list
#' sp.keep <- sl %>%
#'   filter(tolower(English_Common_Name) %in% species |
#'          tolower(Scientific_Name)    %in% species |
#'          AOU %in% species )
#' stopifnot(nrow(sp.keep)>=1)
#'
#' list$observations <- list$observations %>%
#'   filter(AOU %in% sp.keep$AOU)
#' stopifnot(nrow(list$observations)>=1)
#'
#'
#' # TEMPORAL SUBSETTING -----------------------------------------------------
#' list$observations <- list$observations %>% filter(Year %in% year.range)
#' stopifnot(nrow(list$observations)>=2)
#'
#' # BBS PROTOCOL SUBSETTING -----------------------------------------------------
#' list$weather <- list$weather %>% filter(QualityCurrentID %in% QualityCurrentID)
#' if(active.only) list$routes  <- list$routes %>% filter(Active==1)
#'
#'
...
#' rteno.filter <- Reduce(intersect, list(list$observations$RTENO,list$routes$RTENO,list$weather$RTENO, list$vehicle_data$RTENO))
#' # filter out of all relevant data frames..
#' for(i in seq_along(list)){
#'   if(!"RTENO" %in% names(list[[i]]))next()
#'   list[[i]] <- list[[i]] %>% filter(RTENO %in% rteno.filter)
#' }
#'
#'
#' # NEXT --------------------------------------------------------------------
...
#'         # filter out year.range
#'         filter(Year >= min(year.range) & Year <= max(year.range))
#'
#'
...
#'       )
#'
#'     # grab the unique AOU codes associated with species in "species"
#'     list$species_list <- list$species_list %>%
#'       mutate(across(starts_with("English_Common_Name"), tolower)) %>%
#'       filter(across(any_of("English_Common_Name"), ~ .x %in% tolower(species)))
#'
#'     # use the aou for target species to zero-fill the data
#'     myspecies.obs <- list$observations %>%
...
#'     species <- species[tolower(species) %in% list$species_list]
#'     if (zero.fill &
#'         length(species) > 1) {
#'       stop(
...
#'     }
#'     if (zero.fill) {
#'       # unused <- setdiff(list$observations, myspecies.obs) ## setdiff takes twice as long as anti_join
#'       ## create a data frame comprising the route obs for NON TARGET SPECIES (i.e. the zeroes)
#'       zeroes <-
#'         anti_join(list$observations, myspecies.obs) %>% ## anti_join twices as fast as setdiff in this situation
#'         distinct(RouteDataID, Year, RTENO, .keep_all = TRUE) %>%
#'         mutate(AOU = unique(myspecies.obs$AOU)[1]) %>%  #apply target species to create zeroes
...
#'     list$observations <-
#'       bind_rows(myspecies.obs, zeroes) %>% distinct(RTENO, AOU, Year, .keep_all = TRUE)
#'
#'
...
#'
#'
#'    cat("The following species are in your BBS data: ", unique(species), sep = "\n")
#'
#'   return(df)
#'
#'   }
