...
munge_ebird_data <-
  function(fns.ebird,
    ...
    ind <-
      fns.ebird[stringr::str_detect(pattern = "ebird_filtered.rds", fns.ebird)]
    if (length(ind) >= 1) {
      # if overwriting the data, remove this from filelist to be safe..
      if (overwrite) {
        fns.ebird <- setdiff(fns.ebird, ind)
      }
      ...
      if (!overwrite) {
        cat(
          "File ",
          ind,
          "exists. Importing. If you need to re-create the ebird data, specify overwrite=TRUE in `munge_ebird_data()`.\n"
        )
        output <- readRDS(ind)
        return(output)
      }
    ...
    if (length(fns.ebird) < 2)
      stop(
        "check arg `fns.ebird`: must have at least two files (one for sampling events, one for observations."
      )
    if (!any(file.exists(fns.ebird)))
      stop("One or more files specified in `fns.ebird` does not exist")

    ## filenames to import
    f_samp_in <-
      fns.ebird[stringr::str_detect(fns.ebird, "sampling_rel")]
    ...
    if (!is.null(countries)) {
      rc <- rc[tolower(rc$iso_a2) %in% tolower(countries), ]
    }
    ...
    if (nrow(rc) < 1)
      stop(
        "Please ensure arguments `states` and `countries` contain elements witin bbsAssistant::region_codes. See ?filter_ebird_data for further instruction."
      )
    ...
    if (file.exists(f_samp_out) & !overwrite) {
      cat(
        "Overwrite is FALSE and filtered sampling events data already exists. Importing from file (",
        f_samp_out,
        ")\n"
      )
      ...
      cat(
        "Importing the sampling events dataset. This file should take ~2 mins to import. Ignore parsing errors.\n\n"
      )
      ...
      cat(
        "Filtering the sampling events. Takes a few more minutes, depending on size of desired data (i.e., spatial, temporal extents and number of species..\n"
      ...
      sampling <-
        sampling[names(sampling) %in% cols.keep]#keep only useful columns
      ...
      if (!all(years %in% unique(sampling$year)))
        warning("Note that not all years in arg `years` were found in the sampling events dataset.\n")
      if (!is.null(years))
        sampling <- sampling |>
      ...
      cat(
        "Running an alternative to `auk::auk_unique()` on checklists. This takes quite a few minutes for more than a few states/provinces. \n"
      )
      ...
      for (i in seq_along(date.chunk)) {
        ...
        if (i == 1)
          mylist <- list()
        d.ind <-
          as.integer(unlist(date.chunk[i] |> stringr::str_split(pattern = ":")))
        d.min <- dates[min(d.ind)]
        d.max <- dates[max(d.ind)]
        tempdat <-
          sampling[sampling$observation_date >= d.min &
                     sampling$observation_date <= d.max,]
        mylist[[i]] <-
          auk::auk_unique(tempdat, checklists_only = TRUE)
    ...
    cols_obs <-
      cols_samp[!names(cols_samp) %in% c("country",
    ...
    if (file.exists(f_obs_out) & !overwrite) {
      ...
      cat(
        "Overwrite is FALSE and filtered observations data already exists. Importing from file (",
        f_obs_out,
        ")\n"
      )
      ...
      observations <-
        data.table::fread(f_obs_out) # no huge difference when files are small.
    } else{
      #    browser()
      ...
      stopifnot("no species in arg `species` found in observations data" =
                  nrow(observations) > 0)
      ...
      observations <-
        observations |> dplyr::distinct(observer_id,
                                        common_name,
                                        observation_count,
                                        observation_date,
                                        .keep_all = TRUE)
      ...
      observations <- observations |> dplyr::mutate(
        ...
        # must use NA_character_ instead of NA!
        # effort_distance_km to 0 for non-travelling counts
        effort_distance_km = dplyr::if_else(
          protocol_type %in% c("stationary", "Stationary", "STATIONARY"),
          0,
          effort_distance_km
        )
      )
      ...
      cat("Writing the filtered observations data to:",
          f_obs_out,
          "...\n\n")
    ...
    ebird_filtered <-
      list(
        "observations" = dplyr::as_tibble(observations),
    ...
    cat("Writing the filtered and zero-filled eBird data to:",
        f.out,
        "...\n\n")
