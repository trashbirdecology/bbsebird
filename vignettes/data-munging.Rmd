...
# bbs <- make_bbs_data()
fns.bbs.in <-  list.files(dir.bbs.out, pattern = "bbs_obs.rds",recursive = TRUE, full.names = TRUE)
...
# Overlay BBS and study area/sampling grid
  bbs_spatial <- make_bbs_spatial(bbs_obs,
                          ...
                          crs.target = crs.target,
                          grid = grid,
                          dir.out = dir.spatial.out,
                          overwrite = overwrite.bbs
                          )
```
# Step 3: eBird
```{r ebird}
# fns.ebird.in <- list.files(dir.ebird.out,
#                             full.names = TRUE,
#                             recursive = TRUE)
