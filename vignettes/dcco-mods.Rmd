...
```{r setup}
devtools::install_github('trashbirdecology/dubcorms', force=TRUE)
library(dubcorms)
library(tidyverse)
...
```{r retrieve-data}
# Import the data
if(!sim.data) jdat <- import_jdat(dir.jags)
if(sim.data)  bbs  <- dubcorms::sim_bbs(ngrid = 10, maxgridperroute = 2)
```

# Step 1: Specify GAMs
```{r get-gam-data-for-jags}
...
jdat$yBBS.grid %>% dim()   # route by time by grid
jdat$yeBird.grid %>% dim() # checklist by time by grid

jagam.mods$bbs$jags.data$X %>% dim()
jdat$


# jagam.mods$ebird$jags.data %>% str()
# jagam.mods$bbs$jags.data %>% str()
