...
params:
  ...
  dir.proj: "C:/Users/jburnett/documents/github/dubcorms/vignettes/DCCO/"
...
list <- scan_files(dir.proj = dir.proj, scan.for = c("ebird_spatial", "bbs_spatial", "grid", "jags"))
...
j.ind <- ifelse(j.temp|(("jdat" %in% ls()) & overwrite.jdat) | (exists("jdat") & overwrite.jdat), TRUE, FALSE)
...
jdat <- make_jags_list(dat=list(ebird_spatial, grid, bbs_spatial), 
                       dir.out=dir.jags, 
                       max.C.ebird = 100)
message("..done.")

ebird_struc <- get_data_structure(list=jdat$ebird)
bbs_struc <- get_data_structure(list=jdat$bbs)
...
<!-- View the contents and structure of `jdat` (currently needs work see FUN.r for details) -->
<!-- ```{r jdat-structure} -->
...
<!-- ```{r make-gam-dat} -->
<!-- # jagam.data <- make_gam_dat(dat = list(jdat, bbs_spatial), dir.out = dir.jags) -->
